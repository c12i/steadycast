...
export default function PreviewWindow() {
  ...
  useEffect(() => {
    ...
    invoke<PreviewConfig>("get_preview_config")
      .then((cfg) => {
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      if (!xfadingRef.current && !isStreamingRef.current) crossfadeToNext();
    };
    ...
    listen<TrackChangedPayload>("track-changed", (event) => {
      setInfo((prev) =>
        prev
          ? {
              ...prev,
              music_path: event.payload.music_path,
              current_track_index: event.payload.track_index,
            }
          : prev
      );
  ...
  if (!videoPath) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-3 bg-zinc-950 text-zinc-500">
        <div className="h-3 w-3 rounded-full bg-zinc-700" />
        <p className="text-sm">No preview available</p>
        <p className="text-xs text-zinc-600">
          Select a video and click "Pop out" in the main window
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-black">
      {isImagePath(videoPath) ? (
        <img src={convertFileSrc(videoPath)} className="h-full w-full object-cover" alt="" />
      ) : (
        <video
          ref={videoRef}
          autoPlay
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
        <div className="flex items-center gap-2">
          <span
            className={`h-2 w-2 shrink-0 rounded-full ${isStreaming ? "animate-pulse bg-green-400" : "bg-purple-400"}`}
          />
          <span className="truncate text-xs font-medium text-white">
            {isStreaming
              ? trackName || "Live"
              : trackName || (videoPath?.split(/[\\/]/).pop() ?? "Preview")}
          </span>
          {isStreaming && (
            <span className="ml-auto text-[10px] font-semibold tracking-wide text-green-400">
              LIVE
            </span>
          )}
        </div>
