...
import { AmbientEngine, AmbientType, AMBIENT_PRESETS, fetchAmbientAsBase64 } from "../lib/AmbientEngine";
...
export function useAmbient({ onAmbientSaved, onAssetsChanged }: Callbacks) {
  ...
  const togglePreview = useCallback(async (type: AmbientType) => {
    ...
    if (previewingType === type) {
      ...
      try {
        ...
        engineRef.current = null;
        setPreviewingType(null);
        setPreviewError(`Could not load ${type}.mp3 — ensure the file is in public/assets/ambient/`);
      }
    }
  }, [previewingType]);
  ...
  const renderAndSave = useCallback(async (type: AmbientType): Promise<UserAsset | null> => {
